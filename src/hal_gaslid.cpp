...
void errorHandler(hal_err_t err){
    ...
    switch(err){
        ...
        case HAL_ERR_AIN4_20:
            ESP_LOGE("HAL", "AIN4_20 Module error");
            break;

        case HAL_ERR_KMETER:
            ESP_LOGE("HAL", "Kmeter Module error - Themocouple disconnected?");
            break;
...
void Outputs::init() {
    ...
    if(!moduleIO.begin(&Wire, PIN_SDA_I2C_EXT, PIN_SCL_I2C_EXT, MODULE_4IN8OUT_1_ADDR)){
        errorHandler(HAL_ERR_4IN8OUT);
    }
    else ESP_LOGI("HAL", "4IN8OUT Module Found");
    moduleIO.setAllOutput(false);

    if(!gasPumpDriver.begin(&Wire, M5_HBRIDGE_ADDR, PIN_SDA_I2C_EXT, PIN_SCL_I2C_EXT, 100000L)){
        errorHandler(HAL_ERR_HBRIDGE);
    }
    else ESP_LOGI("HAL", "Gas Pump HBRIDGE Module Found");
    gasPumpDriver.setDriverDirection(1);
    gasPumpDriver.setDriverSpeed8Bits(0);
...
void Outputs::setFlowValve(int index, bool ValveState) {
    ...
    bool result = moduleIO.setOutput(index, ValveState);
    ESP_LOGI("HAL", "Flow Valve %d set to %d, success=%d", index, ValveState, result);

}

void Outputs::setGasPumpSpeed(float percent) {
    int speed = percent/100 * 255;
    gasPumpDriver.setDriverSpeed8Bits(speed);
}
...
void Inputs::init(void){
    ...
    if(!gascardReader.begin(&Wire, M5_4_20MA_ADDR, PIN_SDA_I2C_EXT, PIN_SCL_I2C_EXT, 100000L)){
        errorHandler(HAL_ERR_AIN4_20);
    }
    else ESP_LOGI("HAL", "Gascard Reader 4-20mA Module Found");

    if(!thermocoupleReader.begin(&Wire, M5_KMETER_ADDR, PIN_SDA_I2C_EXT, PIN_SCL_I2C_EXT, 100000L)){
        errorHandler(HAL_ERR_AIN4_20);
    }
    else ESP_LOGI("HAL", "Thermocouple Reader Module Found");
    thermocoupleReader.setKmeter(0);
...
void Inputs::pollSensorData(void){
    ...
    inputData.temperatureData["ts1"] = readThermocouple(0); 
    // vTaskDelay(100 / portTICK_PERIOD_MS);
    // inputData.temperatureData["ts2"] = readThermocouple(1); 
    ...
    float current;
    float gasPercent;
    current = (float)(gascardReader.getCurrentValue(0)) / 100.0;
    gasPercent = (current - 4) / 16 * 100;
    ESP_LOGI("HAL", "Gas Card 0: %0.2fmA = %02f CO2(%)", current, gasPercent);
    inputData.gasData["CO2"] = gasPercent;

    current = (float)(gascardReader.getCurrentValue(1)) / 100.0;
    gasPercent = (current - 4) / 16 * 3000;
    ESP_LOGI("HAL", "Gas Card 1: %0.2fmA = %02f N2O(ppm)", current, gasPercent);
    inputData.gasData["N2O"] = gasPercent;

    current = (float)(gascardReader.getCurrentValue(2)) / 100.0;
    gasPercent = (current - 4) / 16 * 100;
    ESP_LOGI("HAL", "Gas Card 2: %0.2fmA = %02f CH4(%)", current, gasPercent);
    inputData.gasData["CH4"] = gasPercent;
...
float Inputs::readThermocouple(int channel){
    //Reads the thermocouple on the specified channel
    //Returns the temperature in degrees C
    thermocoupleReader.setKmeter(channel);
    int error_status = thermocoupleReader.getReadyStatus();
    if (error_status == 0) {
        return ((float)(thermocoupleReader.getCelsiusTempValue())) / 100;
    }
    else{
        errorHandler(HAL_ERR_KMETER);
        return nan("0");
    }
}
